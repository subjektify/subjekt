...
import { AstTargetVisitor } from "./AstTargetVisitor";

export class AstShapeDefinitionVisitor
  ...
  implements SubjektVisitor<[string, AstShape]>
{
  ...
  public visitShapeDefinition(ctx: ShapeDefinitionContext): [string, AstShape] {
  ...
  private _visitShapeDefinition(ctx: ShapeDefinitionContext): AstShape {
  ...
  // Simple shapes
  private _visitSimpleShapeDefinition(ctx: ShapeDefinitionContext): AstShape {
  ...
  // Aggregate shapes
  private _visitAggregateShapeDefinition(
    ctx: ShapeDefinitionContext,
  ): AstShape {
    const shapeType = ctx.shapeType().text;
  ...
  private _visitEnumShape(ctx?: ShapeMembersContext): AstEnumShape {
    const enumMembers = ctx?.enumMembers()?.enumMember();
    let members: Record<string, AstEnumMember> = {};
  ...
  private _visitListShape(ctx?: ShapeMembersContext): AstListShape {
  ...
  private _visitMapShape(ctx?: ShapeMembersContext): AstMapShape {
  ...
  private _visitStructureShape(ctx?: ShapeMembersContext): AstStructureShape {
  ...
  // Subject shapes
  private _visitSubjectShapeDefinition(ctx: ShapeDefinitionContext): AstShape {
    const shapeType = ctx.shapeType().text;
    const shapeMembers = ctx.shapeTypeDefinition()?.shapeMembers();
    switch (shapeType) {
      case "subject":
  ...
  private _visitSubjectShape(ctx?: ShapeMembersContext): AstSubjectShape {
  ...
  private _visitBehaviorShape(ctx?: ShapeMembersContext): AstBehaviorShape {
  ...
  private _visitEventShape(ctx?: ShapeMembersContext): AstEventShape {
    const members = this._members(ctx?.member());
    return {
      type: "event",
      members,
  ...
  private _visitErrorShape(ctx?: ShapeMembersContext): AstErrorShape {
    const members = this._members(ctx?.member());
    return {
      type: "error",
      members,
